...
> **Status: 0.14.0 — feature-complete, full conformance on spec v0.16.0 (+ the knownFunctionNames host API); TeeBox composite-builds this repo's working tree** (its `settings.gradle` includeBuilds `../propertee2-java`, so TeeBox rides HEAD — its full suite was verified green against 0.11.0 on 2026-07-07, 187 tests; the old "TeeBox pins 0.2.x" note is obsolete). All design phases (spike → PA–PF) plus the TeeBox v1-API realignment (R1–R6) are done. The engine passes **all 117 `.tee/.expected` fixtures** (byte-for-byte, deterministic, no flakiness; **310 tests green**): JDK 25 Gradle build + ANTLR codegen, the `value/` model, the full builtin catalog (incl. host-gated ENV/file I/O, `SHELL`/`SHELL_CTX`, and `HTTP`/`HTTP_GET`/`HTTP_POST`), a recursive tree-walk interpreter (`interp/`), the **production Coop cooperative runtime** (`coop/`, `ScopedValue`) with the program as the root fiber + the `Coop.blocking` host contract, **`multi`/`thread`/`monitor`**, external-function registration, keyword/function hiding, an `Engine` embedding API, and a `propertee2` CLI.
>
...
>
> **0.13.0 — knownFunctionNames host API shipped** (TeeBox editor-lint request; no language/spec change — same release class as 0.9.0's validate): `Builtins.names()` (catalog snapshot), façade `BuiltinFunctions.knownFunctionNames()` (dispatch + full catalog + instance host builtins; wiring-independent), `Engine.INTERPRETER_DISPATCHED_NAMES` extracted as the single source for `requireReplaceableName`. Rationale: ALL-CAPS is reserved (spec v0.12.0), so an unknown ALL-CAPS call is a guaranteed call-time failure — hosts get a zero-false-positive typo lint (TeeBox 1.13.0 consumes it). Additive; **113 fixtures / 302 tests green.**
>
> **0.14.0 — spec v0.16.0 "the monitor is a watchdog thread" shipped** (user proposal — ProperTee `docs/design-draft-monitor-watchdog.md`): `monitor` re-specified as an auto-spawned **watchdog thread** — `monitor 500` = sleep 500 ms *between* iterations (fixed-delay; this runtime's fiber loop already did exactly that, so the spec now describes the implementation), worker-grade purity, and **capture** observation. Per iteration: fresh function-style local frame (assignment/loops/function calls now legal — the blanket assignment ban is retired, and a latent bug hiding function params in monitor-called functions is structurally fixed), result var binds a `Values.deepCopy` **captured snapshot** (suspension-consistent, mutation-proof), `::` write = the worker error, bare-name global fallthrough removed. Persistent cross-iteration locals were **rejected** (no init idiom in the language — an uninitializable counter). Unchanged: sleep-first cadence, guaranteed final iteration, `[MONITOR ERROR]` flow. Mildly breaking (error-relying/bare-read corners only); v1 out of scope, js synced best-effort (118 pending — its monitor body can't suspend). Fixture 32 rewritten, 117–118 added; **115 fixtures / 306 tests green, deterministic.**
...
./gradlew dist && java -jar dist/propertee2-0.13.0.jar -p '{"width":100}' script.tee
```
...
- The language spec is canonical in `flatide/ProperTee` LANGUAGE.md; the local `docs/LANGUAGE.md` is a runtime-annotated copy that must be content-synced (not byte-copied) on every spec change — a stale copy shipping opposite semantics is a release defect. The value contract is canonical in `docs/value-model-and-builtins.md`.
