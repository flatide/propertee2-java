...
> **Status: PC done; PD (multi/thread/monitor) next.** The real engine now spans: JDK 25 Gradle build + ANTLR codegen, the `value/` model, the full PURE-builtin catalog, a recursive tree-walk interpreter (`interp/`), and the **production Coop cooperative runtime** (`coop/`, `ScopedValue`-based) with the interpreter running as the root fiber + the `Coop.blocking` host contract. **48 conformance fixtures pass** (46 sequential + ENV + file I/O). Remaining: **PD** (multi/thread/monitor — the ~30-fixture `PENDING` block) plus the host tail (SHELL/HTTP, tasks, external-function registration, keyword/function hiding). The throwaway **spike** (`spike/`) that validated the cooperative model is documented in `docs/spike-findings.md`.
