...
- **PC in progress.** ✅ Production Coop runtime in `coop/` (`Scheduler`/`Fiber`/`FiberState`/`Coop`) — ported from the hardened spike, deterministic round-robin, wake timer, `yield`/`sleep`/`blocking`, **`ScopedValue`** per-fiber context (§5). ✅ The interpreter runs as the **root fiber** (`Engine` → `coop.run`); `SLEEP` → `coop.sleep`; host-gated/blocking builtins route through `coop.blocking` (the §3.1 contract). ✅ `PlatformProvider` (`host/`) + `ENV` → `83_type_env` passes. **Remaining:** file I/O builtins (`85_file_io`), then `SHELL`/tasks/external-functions/keyword-ignore.
