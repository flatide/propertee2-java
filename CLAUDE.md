...
>
> **0.4.0 — spec v0.8.0 first-class `null` shipped** (ProperTee issue #4): `null` is the seventh value type under a "**no implicit null**" principle — the language never produces it (missing args / bare `return` stay `{}`); it enters only via the `null` literal or data (`JSON_PARSE`, host values), making JSON round-trips lossless. Represented by the immutable `value/JsonNull.NULL` singleton; misuse (conditions/logic/arith/member access) fails loudly via the v0.7.0 strictness. Breaking: `null` reserved + `JSON_PARSE` stops normalizing `null`→`{}`. **94 fixtures / 249 tests green, deterministic.**
...
./gradlew test --tests 'com.flatide.propertee2.conformance.ConformanceTest'   # all 94 fixtures vs .expected
...
./gradlew dist && java -jar dist/propertee2-0.4.0.jar -p '{"width":100}' script.tee
```
...
>
> **Spec v0.8.0** (shipped in 0.4.0; issue #4): first-class `null` (`value/JsonNull.NULL`), fixtures 93–96; 84 updated. Everything below is otherwise identical to v1 (= spec v0.6.0).
