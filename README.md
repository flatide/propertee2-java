...
- **상태:** **conformance 스위트 103/103 통과** (byte-for-byte, deterministic). 0.8.0 — **스펙 v0.12.0 대문자 함수 네임스페이스 예약** 구현(breaking — ALL-CAPS 함수 정의는 정의 시점 에러).
...
```bash
./gradlew dist                                  # → dist/propertee2-0.8.0.jar (self-contained fat jar)
java -jar dist/propertee2-0.8.0.jar script.tee

# 호스트 프로퍼티 주입(-p, JSON object → 빌트인 프로퍼티 / _PROPS)
java -jar dist/propertee2-0.8.0.jar -p '{"width":100,"height":200}' script.tee
```
