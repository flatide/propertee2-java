...
```bash
./gradlew dist                                  # → dist/propertee2-0.10.0.jar (self-contained fat jar)
java -jar dist/propertee2-0.10.0.jar script.tee

# 호스트 프로퍼티 주입(-p, JSON object → 빌트인 프로퍼티 / _PROPS)
java -jar dist/propertee2-0.10.0.jar -p '{"width":100,"height":200}' script.tee
```
