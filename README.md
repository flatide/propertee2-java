...
`propertee2-java`는 [ProperTee](https://github.com/flatide/ProperTee) 언어의 **완전 협력형(fully-cooperative) 런타임**이다. 동결된 [`propertee-java`](https://github.com/flatide/propertee-java) v1.0.0(Java 7/8, stepper 기반)이 남긴 eager seam을 **Java 25 virtual thread(Project Loom) 코루틴**으로 근본 해결한다. 값/타입/스코프/에러 메시지 의미는 **스펙에 고정**된다 — v1과 동일한 스펙 v0.6.0을 거쳐, 스펙 v0.7.0(5건의 의도적 breaking 변경: 조건 불리언 엄격화, `and`/`or` 단축 평가, `RANDOM` 단일 인자 폐지, `SLICE` count 규약, `LEN` 엄격화), 현재는 **스펙 v0.8.0**(일급 `null` — 암묵적 null 없이 JSON 무손실 왕복; 마이그레이션 노트는 [`docs/LANGUAGE.md`](docs/LANGUAGE.md) changelog)을 구현한다. 스케줄링(중단/협력)은 v1의 eager seam을 제거한 완전 협력형이다.
...
- **상태:** **conformance 스위트 94/94 통과** (byte-for-byte, deterministic). 0.4.0 — **스펙 v0.8.0 일급 `null`** 구현.
...
```bash
./gradlew build     # 두 모듈 컴파일 + 전체 테스트(conformance 94 fixture 포함)
```

## 실행 (CLI)

```bash
./gradlew dist                                  # → dist/propertee2-0.4.0.jar (self-contained fat jar)
java -jar dist/propertee2-0.4.0.jar script.tee

# 호스트 프로퍼티 주입(-p, JSON object → 빌트인 프로퍼티 / _PROPS)
java -jar dist/propertee2-0.4.0.jar -p '{"width":100,"height":200}' script.tee
```
