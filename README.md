...
- **상태:** **conformance 스위트 90/90 통과** (byte-for-byte, deterministic). 0.3.0 — **스펙 v0.7.0 breaking 배치** 구현.
...
```bash
./gradlew build     # 두 모듈 컴파일 + 전체 테스트(conformance 90 fixture 포함)
```

## 실행 (CLI)

```bash
./gradlew dist                                  # → dist/propertee2-0.3.0.jar (self-contained fat jar)
java -jar dist/propertee2-0.3.0.jar script.tee

# 호스트 프로퍼티 주입(-p, JSON object → 빌트인 프로퍼티 / _PROPS)
java -jar dist/propertee2-0.3.0.jar -p '{"width":100,"height":200}' script.tee
```
