...
final class MultiBuilder {
    ...
    record Pending(String key, String funcName, List<Object> args) {}
    ...
    void addNamed(String key, String funcName, List<Object> args, int line, int col) {
        if (!usedKeys.add(key)) {
            throw new TeeError("Duplicate result key '" + key + "' in multi block", line, col);
        }
        pending.add(new Pending(key, funcName, args));
    }

    /** Add an unnamed thread (auto-key {@code #N}). A collision with an explicit key → error at (line, col). */
    void addUnnamed(String funcName, List<Object> args, int line, int col) {
        String key = "#" + (++unnamed);
        if (!usedKeys.add(key)) {
